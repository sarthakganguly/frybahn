...
let spriteRenderers = [
    ...
    ([_, width, x0, y0, x1, y1, x2, y2, x3, y3]) => {
        c.lineWidth = width;
        c.beginPath();
        c.moveTo(x0, y0);
...
let lerpFrame = (a, b, t) => !!a === a ? a :
    Array.isArray(a)
        ? a.map((x, i) => lerpFrame(x, b[i], t))
        : a + t * (b - a);
;
let s_spriteFrameNum = 0;
export let tickSprite = (state) => {
    if (state === 0 /* Rolling */) {
        s_spriteFrameNum = 0;
    }
    if (state === 1 /* Jumping */) {
...
export let renderSprite = (state) => {
    ...
    for (let layerIdx = 0; layerIdx < animData.length; ++layerIdx) {
        ...
        if (layer.length > 1) {
            nextFrameIdx = 1;
            while (nextFrameIdx < layer.length - 1 && layer[nextFrameIdx][0] <= s_spriteFrameNum)
                nextFrameIdx++;
        ...
        if (frame[1]) {
            c.save();
            c.translate(k_fullWidth / 2 + (state.playerPos[0] - state.cameraPos[0]) * k_baseScale * state.cameraZoom, k_fullHeight / 2 + (state.playerPos[1] - state.cameraPos[1]) * k_baseScale * state.cameraZoom);
            c.scale(state.cameraZoom * k_spriteScale, state.cameraZoom * k_spriteScale);
            c.rotate(state.playerRot);
            c.translate(state.spriteScaleX * frame[2], frame[3]);
            c.scale(state.spriteScaleX, 1);
            c.rotate(frame[4]);
            frame[5].forEach((item) => spriteRenderers[item[0]](item));
            c.restore();
        }
    }
};
