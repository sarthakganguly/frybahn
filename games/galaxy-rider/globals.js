...
// ----- SDFs -----
let v2Lift1 = (fn) => (a) => a.map(fn);
let v2Lift2 = (fn) => (a, b) => [fn(a[0], b[0]), fn(a[1], b[1])];
let v2Length = (v) => Math.sqrt(v2Dot(v, v));
let max = Math.max;
let v2Min = v2Lift2(Math.min);
let v2Max = v2Lift2(max);
let v2Abs = v2Lift1(Math.abs);
let v2Sub = (a, b) => v2MulAdd(a, b, -1);
let v2Scale = (a, b) => v2MulAdd([0, 0], a, b);
let roundMerge = (a, b) => Math.max(Math.min(a, b), 0) - v2Length(v2Min([a, b], [0, 0]));
let sdCircle = (P, x, y, r) => v2Length(v2Sub(P, [x, y])) - r;
let sdRotatedBox = (P, x, y, w, h, _th) => {
    let d = v2MulAdd(v2Abs(v2Sub(P, [x, y])), [w, h], -0.5);
    return v2Length(v2Max(d, [0, 0])) + Math.min(Math.max(d[0], d[1]), 0.);
};
let sdCapsule = (p, x0, y0, ra, x1, y1, rb) => {
    let pa = [x0, y0], pb = v2Sub([x1, y1], pa);
    p = v2Sub(p, pa);
    let h = v2Dot(pb, pb), b = ra - rb;
    let q = v2Scale([v2Dot(p, [pb[1], -pb[0]]), v2Dot(p, pb)], 1 / h), c = [Math.sqrt(h - b * b), b];
    q[0] = Math.abs(q[0]);
    let k = c[0] * q[1] - c[1] * q[0], n = v2Dot(q, q);
...
export let sampleLevel = (level, p) => {
    ...
    let normVec = [normb1 - norma1, normd1 - normc1];
    ...
    let xnormVec = [xnormb1 - xnorma1, xnormd1 - xnormc1];
