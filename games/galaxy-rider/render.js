...
export let renderState = (curLevel, saveState, state) => {
    ...
    for (let i = 0; i < curLevelObjectData.length; ++i) {
        ...
        let objScale = curLevelObjectData[i][0] == 1 ? 4 :
            curLevelObjectData[i][0] == 2 ? curLevelObjectData[i][4] :
                1;
        c.save();
        c.translate(k_fullWidth / 2 + (x - state.cameraPos[0]) * k_baseScale * state.cameraZoom, k_fullHeight / 2 + (y - state.cameraPos[1]) * k_baseScale * state.cameraZoom);
        c.scale(k_baseScale * state.cameraZoom * objScale, k_baseScale * state.cameraZoom * objScale);
        let red = 10 + (([0, 20 + state.canBeDone | 0, 10])[curLevelObjectData[i][0]]);
    ...
    if (messages[curLevel]) {
        c.save();
        c.textAlign = 'left';
        c.translate(k_fullWidth / 2 - state.cameraPos[0] * k_baseScale * state.cameraZoom, k_fullHeight / 2 - state.cameraPos[1] * k_baseScale * state.cameraZoom);
